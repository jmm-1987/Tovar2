...
def enviar_email_enlace_presupuesto_cliente(solicitud, destinatario, enlace_publico):
    """Envía al cliente un email con enlace seguro al presupuesto."""
    try:
        asunto = f"Revisión de Presupuesto {solicitud.numero_solicitud or solicitud.id}"
        cuerpo = (
            f"Hola {solicitud.cliente.nombre if solicitud.cliente else ''},\n\n"
            f"Te compartimos tu presupuesto para revisión.\n\n"
            f"Enlace seguro (caduca):\n{enlace_publico}\n\n"
            "Desde ese enlace podrás aceptar, sugerir cambios o rechazar.\n\n"
            "Gracias."
        )
...
def enviar_email_notificacion_respuesta_cliente(solicitud, comentario):
    """Notifica internamente que el cliente respondió un presupuesto."""
    try:
        destinatarios = []
        email_interno = os.environ.get('INTERNAL_NOTIFICATION_EMAIL', '').strip()
        if email_interno:
            destinatarios.append(email_interno)

        correo_comercial = ''
        if solicitud.comercial and solicitud.comercial.usuario:
            correo_comercial = (solicitud.comercial.usuario.correo or '').strip()
        if correo_comercial and correo_comercial not in destinatarios:
            destinatarios.append(correo_comercial)

        fallback = current_app.config.get('MAIL_DEFAULT_SENDER', '')
        if not destinatarios and fallback:
            destinatarios.append(fallback)
        if not destinatarios:
            return False, 'Sin destinatarios internos configurados'

        enlace = f"{request_root_url_safe()}/presupuestos/{solicitud.id}#modificaciones-cliente"
        asunto = f"Cliente respondió Presupuesto {solicitud.numero_solicitud or solicitud.id}"
        historial_extra = ''
        try:
            raw = getattr(solicitud, 'historial_respuestas_cliente', None) or ''
            hist = json.loads(raw) if raw.strip() else []
            if isinstance(hist, list) and hist:
                historial_extra = f"\nHistorial de respuestas ({len(hist)} entradas):\n"
                for ent in hist[-8:]:
                    historial_extra += (
                        f"  • {ent.get('en', '')} — {ent.get('subestado', '')} — "
                        f"{(ent.get('comentario') or '(sin comentario)')[:400]}\n"
                    )
        except (TypeError, ValueError, json.JSONDecodeError):
            pass
        cuerpo = (
            f"El cliente respondió el presupuesto {solicitud.numero_solicitud or solicitud.id}.\n\n"
            f"Última respuesta: {solicitud.cliente_respuesta or '-'}\n"
            f"Subestado: {solicitud.subestado or '-'}\n"
        ...
        mail.send(msg)
        return True, 'Notificación interna enviada'
    except Exception as e:
        return False, f'Error al enviar notificación interna: {str(e)}'


def request_root_url_safe():
    """Obtiene URL base de la app sin depender del contexto de request."""
