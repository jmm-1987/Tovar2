...
app.config['CLIENT_TOKEN_EXPIRY_DAYS'] = int(os.environ.get('CLIENT_TOKEN_EXPIRY_DAYS', '7'))
# Temporal: permitir reenviar a FacturaDirecta aunque ya exista fd_deliverynote_uuid
app.config['FACTURADIRECTA_ALLOW_RESEND'] = (
    (os.environ.get('FACTURADIRECTA_ALLOW_RESEND') or '').strip().lower() in ('1', 'true', 'yes', 'on')
)
...
def inject_cliente_respuestas_pendientes():
    """Inyecta contador de respuestas de cliente para mostrar en barra superior."""
    from flask_login import current_user
    from models import Presupuesto
    if not current_user.is_authenticated:
        return {'respuestas_cliente_pendientes': 0, 'respuesta_cliente_ultimo_id': None}
    try:
        # Pendiente = hay respuesta y (nunca vista o el cliente volvió a responder después de la última lectura)
        pendientes_q = Presupuesto.query.filter(
            Presupuesto.cliente_respondido_at.isnot(None),
            Presupuesto.cliente_respuesta.isnot(None),
            or_(
                Presupuesto.respuesta_cliente_notif_vista_at.is_(None),
                Presupuesto.respuesta_cliente_notif_vista_at < Presupuesto.cliente_respondido_at,
            ),
        ).order_by(Presupuesto.cliente_respondido_at.desc())
        pendientes = pendientes_q.all()
        return {
            'respuestas_cliente_pendientes': len(pendientes),
            'respuesta_cliente_ultimo_id': pendientes[0].id if pendientes else None
        }
    except Exception:
        return {'respuestas_cliente_pendientes': 0, 'respuesta_cliente_ultimo_id': None}
...
from routes.solicitudes import presupuestos_bp
    ...
    with app.app_context():
        try:
            ...
            if 'presupuestos' in table_names:
                ...
                # FacturaDirecta: columnas para guardar albarán en FD (desde presupuestos)
                for col, ddl in [
                    ('fd_deliverynote_uuid', 'VARCHAR(64)'),
                    ('fd_deliverynote_doc_number', 'VARCHAR(100)'),
                    ('fd_deliverynote_sent_at', 'TIMESTAMP'),
                    ('fd_deliverynote_last_error', 'TEXT'),
                ]:
                    if col not in columns_presupuestos:
                        try:
                            with db.engine.connect() as conn:
                                conn.execute(text(f'ALTER TABLE presupuestos ADD COLUMN {col} {ddl}'))
                                conn.commit()
                                print(f"Migración: Columna {col} agregada exitosamente a presupuestos")
                        except Exception as e:
                            print(f"Error al agregar columna {col} a presupuestos: {e}")
            ...
            if 'facturas' in table_names:
                ...
                # FacturaDirecta: columnas para guardar albarán en FD (en albaranes de retirada)
                for col, ddl in [
                    ('fd_deliverynote_uuid', 'VARCHAR(64)'),
                    ('fd_deliverynote_doc_number', 'VARCHAR(100)'),
                    ('fd_deliverynote_sent_at', 'TIMESTAMP'),
                    ...
                    if col not in columns_facturas:
                        try:
                            with db.engine.connect() as conn:
                                conn.execute(text(f'ALTER TABLE facturas ADD COLUMN {col} {ddl}'))
                                conn.commit()
                                print(f"Migración: Columna {col} agregada exitosamente a facturas")
                        except Exception as e:
                            print(f"Error al agregar columna {col} a facturas: {e}")
            ...
            if 'presupuestos' not in table_names:
                ...
                # Campos para flujo de aprobación de cliente en portal público
                columnas_flujo_cliente = {
                ...
                for columna, tipo in columnas_flujo_cliente.items():
                    if columna not in columns_presupuesto:
                        try:
                            with db.engine.connect() as conn:
                                conn.execute(text(f'ALTER TABLE presupuestos ADD COLUMN {columna} {tipo}'))
...
{empresa_nombre}'''
                ...
                # Plantillas específicas por estado de presupuesto
                {
...
{empresa_nombre}'''
                },
                {
                    ...
                    'asunto': 'Presupuesto #{solicitud_id} aceptado',
                    'cuerpo': '''Estimado/a {cliente_nombre},

Su presupuesto #{solicitud_id} ha sido ACEPTADO.
...
{empresa_nombre}'''
                },
                {
                    ...
                    'asunto': 'Presupuesto #{solicitud_id} - Mockup en proceso',
                    'cuerpo': '''Estimado/a {cliente_nombre},

Su presupuesto #{solicitud_id} ha pasado al estado MOCKUP.
...
{empresa_nombre}'''
                },
                {
                    ...
                    'asunto': 'Presupuesto #{solicitud_id} en preparación',
                    'cuerpo': '''Estimado/a {cliente_nombre},

Su presupuesto #{solicitud_id} ha pasado al estado EN PREPARACIÓN.
...
{empresa_nombre}'''
                ...
                # Plantillas para subestados de "en preparacion"
                {
                    ...
                    'asunto': 'Presupuesto #{solicitud_id} - Hacer Marcada',
                    'cuerpo': '''Estimado/a {cliente_nombre},

Su presupuesto #{solicitud_id} ha avanzado en el proceso de preparación.
...
{empresa_nombre}'''
                },
                {
                    ...
                    'asunto': 'Presupuesto #{solicitud_id} - Impresión',
                    'cuerpo': '''Estimado/a {cliente_nombre},
...
{empresa_nombre}'''
                },
                {
                    ...
                    'asunto': 'Presupuesto #{solicitud_id} - Calandra',
                    'cuerpo': '''Estimado/a {cliente_nombre},
...
{empresa_nombre}'''
                },
                {
                    ...
                    'asunto': 'Presupuesto #{solicitud_id} - Corte',
                    'cuerpo': '''Estimado/a {cliente_nombre},
...
{empresa_nombre}'''
                },
                {
                    ...
                    'asunto': 'Presupuesto #{solicitud_id} - Confección',
                    'cuerpo': '''Estimado/a {cliente_nombre},
...
{empresa_nombre}'''
                },
                {
                    ...
                    'asunto': 'Presupuesto #{solicitud_id} - Sublimación',
                    'cuerpo': '''Estimado/a {cliente_nombre},
...
{empresa_nombre}'''
                },
                {
                    ...
                    'asunto': 'Presupuesto #{solicitud_id} - Bordado',
                    'cuerpo': '''Estimado/a {cliente_nombre},
...
{empresa_nombre}'''
                },
                {
                    ...
                    'asunto': 'Presupuesto #{solicitud_id} en revisión y empaquetado',
                    'cuerpo': '''Estimado/a {cliente_nombre},

Su presupuesto #{solicitud_id} se encuentra en estado REVISIÓN Y EMPAQUETADO.
...
{empresa_nombre}'''
                },
                {
                    ...
                    'asunto': 'Presupuesto #{solicitud_id} entregado',
                    'cuerpo': '''Estimado/a {cliente_nombre},

Su presupuesto #{solicitud_id} ha sido ENTREGADO.
...
{empresa_nombre}'''
            ...
            try:
                ...
                estados_validos = ['presupuesto', 'rechazado', 'cancelado', 'aceptado', 'mockup', 'en preparacion', 'revision y empaquetado', 'entregado al cliente']
            ...
            # Verificar si existe la tabla facturas y agregar columnas si no existen
            if 'facturas' in table_names:
                columns_facturas = [col['name'] for col in inspector.get_columns('facturas')]
                ...
                if 'tipo_iva' not in columns_facturas:
                    try:
                        with db.engine.connect() as conn:
                            conn.execute(text('ALTER TABLE facturas ADD COLUMN tipo_iva NUMERIC(5, 2) DEFAULT 21.00'))
                            conn.commit()
                            print("Migración: Columna tipo_iva agregada exitosamente a facturas")
                    except Exception as e:
                        print(f"Error al agregar columna tipo_iva a facturas: {e}")
