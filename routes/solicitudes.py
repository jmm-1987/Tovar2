...
def preparar_datos_imprimir_solicitud(solicitud_id):
    ...
    for linea in solicitud.lineas:
        # Usar precio_final si existe y tiene descuento, sino precio_unitario
        if linea.precio_final and linea.descuento and linea.descuento > 0:
            precio_unit = Decimal(str(linea.precio_final))
        else:
    ...
    def convertir_imagen_a_base64(ruta_imagen):
        ...
        if not os.path.isabs(ruta_imagen):
            ruta_imagen = os.path.join(current_app.config['UPLOAD_FOLDER'], ruta_imagen)
    ...
    for i in range(1, 6):
        ...
        imagen_nombre = getattr(solicitud, campo_imagen, None)
        if imagen_nombre:
        ...
        # Obtener descripción
        descripcion = getattr(solicitud, campo_descripcion, '') or ''
        descripciones_imagenes.append(descripcion)
    
    return {
        'presupuesto': solicitud,  # Mantener nombre 'presupuesto' para compatibilidad con template
